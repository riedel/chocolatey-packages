...
function global:github_vars($github_repository) {
    ...
    $release = Invoke-RestMethod "https://api.github.com/repos/$github_repository/releases/latest" -Headers $headers
        @{
            repo = Invoke-RestMethod ("https://api.github.com/repos/" + $github_repository) -Headers $headers
            release = $release
            license = Invoke-RestMethod ("https://api.github.com/repos/" + $github_repository + "/license") -Headers $headers
            topics = (Invoke-RestMethod -Headers $topicHeaders "https://api.github.com/repos/$github_repository/topics").names -join " "
            readme = Invoke-RestMethod ("https://api.github.com/repos/" + $github_repository + "/readme") -Headers $headers
            release_author = Invoke-RestMethod $release.author.url -Headers $headers
...
function global:au_SearchReplace {
    ...
    if ($Latest.containsKey("URL32")) {
	    ...
	    if ($Latest.containsKey("file64")) {

		    @{
			    ".\tools\chocolateyInstall.ps1" = @{
				    "(^\s*file64\s*=\s*)(.*)"     = "`$1 (Get-ChocolateyPath -PathType 'PackagePath') + '/$($Latest.file64)'"
			    }
		    }
	    }else{
		    @{
...
function global:au_BeforeUpdate ($package) {
	if (  (Test-Path "pandoc.lua") -and `
		...
		"`n" + (Invoke-WebRequest -UseBasicParsing -Uri $Latest.readmeUrl).Content | `
	...
	if ($Latest.PSObject.Properties.Name -contains "licenseUrl") {
		"`n" + (Invoke-WebRequest -UseBasicParsing -Uri $Latest.licenseUrl).Content |  Out-File -Encoding "UTF8" ($package.Path + "\tools\LICENSE.txt")
	}
